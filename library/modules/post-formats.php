...
function tamatebako_structured_post_formats() {
	...
	if ( current_theme_supports( 'post-formats', 'aside' ) ) {
		add_filter( 'the_content', 'tamatebako_post_format_aside_infinity', 9 ); // Run at 9. Before wpautop.
	}
...
/**
 * Filters the content of the link format posts.
 * Wraps the content in the make_clickable() function so that users can enter just a URL into the post content editor.
 *
 * @since 1.0.0
 *
 * @param string $content Post content.
...
/**
 * Separates the post content into an array of arrays for formatting of the chat content.
 *
 * @since 1.0.0
 *
 * @param string $content Post content.
...
function tamatebako_get_the_post_format_chat( $content ) {
	...
	foreach ( $chat_rows as $chat_row ) {
		...
		if ( preg_match( '/(?<!http|https)' . $separator . '/', $chat_row ) ) {
			...
			$stanza[] = $row;
		} else { // If no speaker is found.

			// Make sure we have text.
			if ( ! empty( $chat_row ) ) {
				$stanza[] = array(
					'message' => trim( $chat_row ),
				);
			}
		}
